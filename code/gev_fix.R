gev_fix <- function (xdat, ydat = NULL, mul = NULL, sigl = NULL, shl = NULL, 
          mulink = identity, siglink = identity, shlink = identity, 
          muinit = NULL, siginit = NULL, shinit = NULL, show = TRUE, 
          method = "Nelder-Mead", maxit = 10000, fix.sh = NULL, ...){
  z <- list()
  npmu <- length(mul) + 1
  npsc <- length(sigl) + 1
  npsh <- length(shl) + 1
  ...
  if (show) {
    if (z$trans) {
      print(z[c(2, 3, 4)])}else {print(z[4])}
    if (!z$conv) {
      print(z[c(5, 7, 9)])}
  }
  class(z) <- "gev.fit"
