fit_exp <- function(x, mu, sig, sh, para=c("double", "single", "s")) {
  ...
  for (i in 1:27) {
    ...
    fit <- try(gev.fit(x, y, mul = mul, sigl = sigl, shl = shl,siglink = exp),silent=TRUE)
    if('try-error' %in% class(fit)){
      ...
      fit <- gev.fit(x, y, mul = mul, sigl = sigl, shl = shl,siglink = exp)
      ...
      fit[["est_sig"]][,2:4] <- exp(fit[["est_sig"]][,2:4])
      name <- paste0("fit", i )
      assign(name, fit)
    }
  }
